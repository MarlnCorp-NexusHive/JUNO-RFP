...
class OperationalExcellenceService {
  ...
  async analyzeOperations(corporateData, analysisType = "efficiency", timePeriod = "quarterly") {
    try {
      console.log("Starting operational excellence analysis with data:", corporateData);
      
      const prompt = this.buildOperationalAnalysisPrompt(corporateData, analysisType, timePeriod);
      ...
      return this.processOperationalAnalysis(response, corporateData, analysisType, timePeriod);
    } catch (error) {
      console.error("Operational excellence analysis error:", error);
      return this.getFallbackOperationalAnalysis(corporateData, analysisType, timePeriod);
    }
  }
