...
import strategicInsightsService from '../../shared/services/strategicInsightsService';
                ...
                <div key={index} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-medium text-gray-900 dark:text-white text-sm">
                      {metric.metric || metric.name || 'Unknown Metric'}
                    </h4>
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${getInsightTypeColor(insight.category || insight.type)}`}>
                        {insight.category || insight.type || 'General'}
                      </span>
                    ...
                    <div key={index} className="flex items-start gap-3">
                      ...
                      <div>
                        <h4 className="font-medium text-gray-900 dark:text-white">
                          {risk.risk || risk.title || 'Unknown Risk'}
                        </h4>
                        <p className="text-sm text-gray-700 dark:text-gray-300">
                          {risk.description || risk.mitigation || 'No description available'}
                        </p>
                        <div className="flex items-center gap-4 mt-2 text-xs text-gray-500 dark:text-gray-400">
                          <span>Probability: {risk.probability || 'Medium'}</span>
                          <span>Impact: {risk.impact || 'Medium'}</span>
                          <span>Mitigation: {risk.mitigation || 'Review and address'}</span>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-3">
                  {insights.opportunities.map((opportunity, index) => (
                    <div key={index} className="flex items-start gap-3">
                      ...
                      <div>
                        <h4 className="font-medium text-gray-900 dark:text-white">
                          {opportunity.opportunity || opportunity.title || 'Unknown Opportunity'}
                        </h4>
                        <p className="text-sm text-gray-700 dark:text-gray-300">
                          {opportunity.description || 'No description available'}
                        </p>
                        <div className="flex items-center gap-4 mt-2 text-xs text-gray-500 dark:text-gray-400">
                          <span>Potential: {opportunity.potential || 'Medium'}</span>
                          <span>Effort: {opportunity.effort || 'Medium'}</span>
                          <span>Timeline: {opportunity.timeline || 'Short-term'}</span>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-3">
                  {insights.recommendations.map((recommendation, index) => (
                    <div key={index} className="flex items-start gap-3">
                      ...
                      <div>
                        <h4 className="font-medium text-gray-900 dark:text-white">
                          {recommendation.recommendation || recommendation.title || 'Strategic Recommendation'}
                        </h4>
                        <p className="text-sm text-gray-700 dark:text-gray-300">
                          {recommendation.description || 'No description available'}
                        </p>
                        <div className="flex items-center gap-4 mt-2 text-xs text-gray-500 dark:text-gray-400">
                          <span>Priority: {recommendation.priority || 'Medium'}</span>
                          <span>Timeline: {recommendation.timeline || 'Short-term'}</span>
                          <span>Resources: {recommendation.resources || 'Standard resources'}</span>
                        </div>
                      </div>
                    </div>
                  ))}
